...
struct Recipe: Hashable, Decodable, Identifiable, Sendable {
  ...
  var shoppingListItems: [ShoppingListModel]
  ...
  init?(dataItem: RecipeItem) {
    ...
    // Initialize shoppingListItems from Core Data
    if let shoppingListItems = dataItem.shoppinglistitems?.allObjects as? [ShoppingListItem] {
      self.shoppingListItems = shoppingListItems.map { ShoppingListModel(dataItem: $0) }
    } else {
